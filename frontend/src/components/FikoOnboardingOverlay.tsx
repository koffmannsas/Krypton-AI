...
export default function FikoOnboardingOverlay({ user, onComplete }: FikoOnboardingOverlayProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      let errorMessage = 'Connexion réseau instable. Recommence, s\'il te plaît.';
      if (error?.status === 429 || error?.toString().includes("RESOURCE_EXHAUSTED")) {
        errorMessage = "Mon système est temporairement suspendu (Quota IA dépassé). Veuillez recharger vos crédits API ou nous contacter directement.";
      }
