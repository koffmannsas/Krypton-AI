...
import FikoConnectPage from "./pages/fiko/FikoConnectPage";
import FikoSendPage from "./pages/fiko/FikoSendPage";
import EgeriePage from "./pages/egerie/EgeriePage";
...
import AmbassadeursPage from "./pages/ambassadeurs/AmbassadeursPage";
import AssistancePage from "./pages/assistance/AssistancePage";
import AboutPage from "./pages/about/AboutPage";
import PortalActivationWorkflow from "./pages/activation/PortalActivationWorkflow";
...
const App: React.FC = () => {
  ...
  const isClientApp =
    hostname === "clients.krypton-ia.tech" || 
    urlParams.get("domain") === "client" || 
          ...
          if (firebaseUser) {
            try {
              ...
              if (userDoc.exists()) {
                ...
                const publicPaths = [
                  ROUTES[Page.HOME].path, 
                  ROUTES[Page.AUTH].path, 
                  ROUTES[Page.SIGNUP].path,
                  "/login",
                  "/register",
                  "/signup",
                  "/auth"
  ...
  const isAuthPage = location.pathname === ROUTES[Page.AUTH].path || 
                     location.pathname === ROUTES[Page.SIGNUP].path || 
                     location.pathname === ROUTES[Page.ADMIN_AUTH].path;
  ...
  const BlogPostWrapper = ({ onNavigate }: { onNavigate: (p: Page) => void }) => {
    ...
    const title = post ? post.title : 'Article Introuvable | Krypton IA';
  ...
  const AssistanceTopicWrapper = () => {
    const { slug } = useParams<{ slug: string }>();
    const config = ROUTES[Page.ASSISTANCE_TOPIC];
    const displayKeyword = slug?.replace(/-/g, " ") || "";
    const title = config.title.replace("%s", displayKeyword);
    const description = config.description.replace("%s", displayKeyword);
    ...
    return (
      <>
        <SEO title={title} description={description} canonical={`/assistance/${slug}`} />
        <AssistancePage />
      </>
    );
  };
  ...
  return (
    <CosmicProvider>
      <div className="min-h-screen bg-black text-white flex flex-col relative">
        ...
        {!isAnyDashboard && !isAuthPage && (
          <>
            <Navbar
              user={user}
              onLogout={handleLogout}
            />
            {/* Permanent Floating Button "🚀 Ouvrir Fiko" */}
            <motion.div
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="fixed bottom-6 right-6 z-[999] flex items-center"
            >
              <a
                href={user ? getSaaSUrl("/dashboard") : getSaaSUrl("/register")}
                onClick={() => {
                  trackEvent("landing_cta_click", { cta_label: "Floating CTA Permanent", is_logged_in: !!user });
                  trackEvent("dashboard_access", { source: "floating_button" });
                }}
                className="flex items-center gap-2.5 px-6 py-4 bg-[#FF2718] hover:bg-neutral-900 border border-white/15 text-white font-black text-xs md:text-sm uppercase tracking-[0.16em] rounded-full shadow-[0_20px_40px_rgba(255,39,24,0.3)] transition-all duration-300"
              >
                <span>🚀 Ouvrir Fiko</span>
              </a>
            </motion.div>
          </>
        )}
        ...
        <main className={`flex-grow ${isAnyDashboard ? "pb-0" : ""}`}>
          <Routes>
            <Route path={ROUTES[Page.HOME].path} element={<PageWrapper config={ROUTES[Page.HOME]}><HomePage onNavigate={navigateTo} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            <Route path="/login" element={<PageWrapper config={ROUTES[Page.AUTH]}><AuthPage onLoginSuccess={handleLoginSuccess} onNavigate={navigateTo} initialMode="login" /></PageWrapper>} />
            <Route path="/register" element={<PageWrapper config={ROUTES[Page.SIGNUP]}><AuthPage onLoginSuccess={handleLoginSuccess} onNavigate={navigateTo} initialMode="signup" /></PageWrapper>} />
            <Route path="/signup" element={<PageWrapper config={ROUTES[Page.SIGNUP]}><AuthPage onLoginSuccess={handleLoginSuccess} onNavigate={navigateTo} initialMode="signup" /></PageWrapper>} />
            <Route path="/auth" element={<PageWrapper config={ROUTES[Page.AUTH]}><AuthPage onLoginSuccess={handleLoginSuccess} onNavigate={navigateTo} initialMode="login" /></PageWrapper>} />

            {/* Auth Routes mapped from ROUTES config */}
            ...
            <Route path={ROUTES[Page.PRICING].path} element={<PageWrapper config={ROUTES[Page.PRICING]}><PricingPage onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            ...
            <Route path={ROUTES[Page.FIKO].path} element={<PageWrapper config={ROUTES[Page.FIKO]}><FikoLandingPage onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenChat={() => setFikoChat({ open: true, agent: MASTER_AGENTS[0] })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            ...
            <Route path={ROUTES[Page.FIKO_CONNECT].path} element={<PageWrapper config={ROUTES[Page.FIKO_CONNECT]}><FikoConnectPage onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} /></PageWrapper>} />
            <Route path={ROUTES[Page.FIKO_SEND].path} element={<PageWrapper config={ROUTES[Page.FIKO_SEND]}><FikoSendPage /></PageWrapper>} />
            <Route path={ROUTES[Page.EGERIE].path} element={<PageWrapper config={ROUTES[Page.EGERIE]}><EgeriePage /></PageWrapper>} />
            ...
            {/* Offer Routes */}
            <Route path={ROUTES[Page.ACCESS_OFFER].path} element={<PageWrapper config={ROUTES[Page.ACCESS_OFFER]}><AccessOfferPage onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            <Route path={ROUTES[Page.TERRA_OFFER].path} element={<PageWrapper config={ROUTES[Page.TERRA_OFFER]}><GateOfferPage gate="TERRA" onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            <Route path={ROUTES[Page.MARS_OFFER].path} element={<PageWrapper config={ROUTES[Page.MARS_OFFER]}><GateOfferPage gate="MARS" onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            <Route path={ROUTES[Page.KRYPTON_OFFER].path} element={<PageWrapper config={ROUTES[Page.KRYPTON_OFFER]}><GateOfferPage gate="KRYPTON" onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            <Route path={ROUTES[Page.GALAXY_OFFER].path} element={<PageWrapper config={ROUTES[Page.GALAXY_OFFER]}><GateOfferPage gate="GALAXY" onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            
            {/* Fiko Addons */}
            <Route path={ROUTES[Page.FIKO_SOLO_OFFER].path} element={<PageWrapper config={ROUTES[Page.FIKO_SOLO_OFFER]}><FikoAddonOfferPage addon="SOLO" onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            <Route path={ROUTES[Page.FIKO_PILOT_OFFER].path} element={<PageWrapper config={ROUTES[Page.FIKO_PILOT_OFFER]}><FikoAddonOfferPage addon="PILOT" onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            <Route path={ROUTES[Page.FIKO_ELITE_OFFER].path} element={<PageWrapper config={ROUTES[Page.FIKO_ELITE_OFFER]}><FikoAddonOfferPage addon="ELITE" onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            <Route path={ROUTES[Page.FIKO_EMPIRE_OFFER].path} element={<PageWrapper config={ROUTES[Page.FIKO_EMPIRE_OFFER]}><FikoAddonOfferPage addon="EMPIRE" onNavigate={navigateTo} onOpenVocal={(gate) => setFikoVocal({ open: true, initialPorte: gate })} onOpenFiko={(gate) => navigate(`/activation?gate=${gate}`)} /></PageWrapper>} />
            
            <Route path={ROUTES[Page.PRIVACY].path} element={<PageWrapper config={ROUTES[Page.PRIVACY]}><PrivacyPage /></PageWrapper>} />
            <Route path={ROUTES[Page.DATA_DELETION].path} element={<PageWrapper config={ROUTES[Page.DATA_DELETION]}><DataDeletionPage /></PageWrapper>} />
            <Route path={ROUTES[Page.TERMS].path} element={<PageWrapper config={ROUTES[Page.TERMS]}><TermsPage /></PageWrapper>} />
            <Route path={ROUTES[Page.AUTH_CALLBACK].path} element={<PageWrapper config={ROUTES[Page.AUTH_CALLBACK]}><AuthCallbackPage /></PageWrapper>} />
            <Route path={ROUTES[Page.AMBASSADEURS].path} element={<PageWrapper config={ROUTES[Page.AMBASSADEURS]}><AmbassadeursPage /></PageWrapper>} />
            <Route path={ROUTES[Page.ASSISTANCE].path} element={<PageWrapper config={ROUTES[Page.ASSISTANCE]}><AssistancePage /></PageWrapper>} />
            <Route path={ROUTES[Page.ASSISTANCE_TOPIC].path} element={<AssistanceTopicWrapper />} />
            <Route path={ROUTES[Page.ABOUT].path} element={<PageWrapper config={ROUTES[Page.ABOUT]}><AboutPage onNavigate={navigateTo} /></PageWrapper>} />
            <Route path={ROUTES[Page.PORTAL_ACTIVATION].path} element={<PageWrapper config={ROUTES[Page.PORTAL_ACTIVATION]}><PortalActivationWorkflow onNavigate={navigateTo} gate={urlParams.get("gate") || selectedGate} /></PageWrapper>} />
            
            {/* Client Profile */}
            <Route path={ROUTES[Page.PROFILE].path} element={<PageWrapper config={ROUTES[Page.PROFILE]}>{user ? <ProfilePage user={user} onUpdateUser={(updatedProfile) => setUser(updatedProfile)} onNavigate={navigateTo} onLogout={handleLogout} /> : <Navigate to="/connexion" />}</PageWrapper>} />

            {/* Client Dashboard */}
            <Route path={ROUTES[Page.CLIENT_DASHBOARD].path} element={<PageWrapper config={ROUTES[Page.CLIENT_DASHBOARD]}>{user ? <ClientDashboardPage user={user} onNavigate={navigateTo} onLogout={handleLogout} gate={selectedGate} onUpdateUser={(updatedProfile) => setUser(updatedProfile)} /> : <Navigate to="/connexion" />}</PageWrapper>} />
        ...
        {!isAnyDashboard && !isAuthPage ? (
          ...
          <div className="sticky bottom-0 z-50 w-full bg-[#0A0A0C] border-t border-[#FF2718]/20 py-4 flex items-center justify-center backdrop-blur-xl">
            <a 
              ...
              className="text-xs font-black uppercase tracking-[0.4em] text-[#FF2718] m-0 drop-shadow-[0_0_8px_rgba(255,39,24,0.3)] hover:opacity-80 transition-opacity cursor-pointer"
            >
