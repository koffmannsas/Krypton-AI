      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden backdrop-blur-md shadow-2xl flex flex-col h-[600px]">
          <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              ...
              <div key={i}>
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-4 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 bg-white/5">
                        {m.role === 'assistant' ? <Bot className="text-[#E10600]" size={20} /> : <User className="text-gray-400" size={20} />}
                    </div>
                    <div className={`p-4 rounded-2xl ${m.role === 'assistant' ? 'bg-white/10 text-white rounded-tl-none' : 'bg-[#E10600] text-white rounded-tr-none'}`}>
                        {m.content.split(/(https?:\/\/[^\s]+|wa\.me\/[^\s]+)/g).map((part, i) => 
                          part.match(/(https?:\/\/[^\s]+|wa\.me\/[^\s]+)/) ? (
                            <a key={i} href={part.startsWith('http') ? part : `https://${part}`} target="_blank" rel="noopener noreferrer" className="underline font-bold text-white hover:text-[#E10600]">
                              {part}
                            </a>
                          ) : (
                            part
                ...
                {m.role === 'assistant' && userTurnCount >= 2 && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-2 ml-14 mt-3">
                        {['Lancer mon analyse', 'Voir les offres', 'Activer mon système'].map(action => (
                            <button key={action} onClick={() => handleCtaClick(action)} className="text-[10px] uppercase tracking-widest border border-[#E10600]/30 hover:bg-[#E10600] px-3 py-1.5 rounded-full text-gray-300 hover:text-white transition-all">
                                {action}
                            </button>
                        ))}
                    </motion.div>
            ...
            {loading && <div className="flex gap-4"><Loader2 className="animate-spin text-[#E10600]" /></div>}
            <div ref={chatEndRef} />
          </div>
          ...
          <div className="p-4 border-t border-white/10">
            <div className="flex gap-2 flex-wrap mb-4">
              {['Comment avoir plus de clients ?', 'Pourquoi mon site ne convertit pas ?', 'Est-ce que ça marche pour mon business ?'].map(s => (
                <button key={s} onClick={() => handleSend(s)} className="text-xs bg-white/5 hover:bg-[#E10600] transition-colors border border-white/10 px-3 py-1.5 rounded-full text-gray-300 hover:text-white">
                  {s}
                </button>
              ))}
            </div>
            <div className="flex gap-2">
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend(input)} className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#E10600]" placeholder="Posez votre question à Fiko..." />
              <button onClick={() => handleSend(input)} className="bg-[#E10600] p-3 rounded-xl"><Send size={20} /></button>
            </div>
          </div>
        </div>
      </div>
